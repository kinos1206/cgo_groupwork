...
class MySchedulerCSO():
    def __init__(self, opt):
        self.activations = ['relu', 'sigmoid', 'hardtanh', 'softmax']
        self.optimizer_choices = ['Adam', 'SGD', 'Adagrad', 'RMSprop']
        self.population_size = 10  # 猫群のサイズ
        self.sum_epoch = 0
        self.min_epoch = opt.epoch_min
        self.limit_epoch = opt.epoch_limit
        self.result = []
        self.seek_mode_ratio = 0.5  # 追捕モードの猫の割合
        self.seek_range = 0.2  # 探索範囲
        self.batchSize = [16, 32, 64, 128]
        self.epoch = [10, 15, 20]
        self.lr = [0.001, 0.002, 0.004, 0.01, 0.02]
        self.activation = [0, 1, 2, 3]
    ...
    # 探索モード
    def seek_mode(self, cat):
        new_cat = cat.copy()
        for key in cat:
            if key == 'activation' or key == 'optimizer':
                continue
            new_cat[key] = cat[key] * (1 + random.uniform(-self.seek_range, self.seek_range))
            if key == 'batchSize' or key == 'epoch':
                new_cat[key] = int(round(max(min(new_cat[key], max(getattr(self, key))), min(getattr(self, key)))))
            elif key == 'lr':
                new_cat[key] = max(min(new_cat[key], max(self.lr)), min(self.lr))
        return new_cat
    
    # 追跡モード
    def trace_mode(self, cat):
        direction = self.best_cat
        new_cat = cat.copy()
        for key in cat:
            if key == 'activation' or key == 'optimizer':
                continue
            new_cat[key] += random.uniform(0, 1) * (direction[key] - cat[key])
            if key == 'batchSize' or key == 'epoch':
                new_cat[key] = int(round(max(min(new_cat[key], max(getattr(self, key))), min(getattr(self, key)))))
            elif key == 'lr':
                new_cat[key] = max(min(new_cat[key], max(self.lr)), min(self.lr))
    ...
    def search(self, index, epoch, opt, history):
        ...
        if fitness > self.best_fitness:
            ...
            self.best_cat = self.population[index]
        ...
        for i in range(len(self.population)):
            if random.random() < self.seek_mode_ratio:
                self.population[i] = self.seek_mode(self.population[i])
            else:
                self.population[i] = self.trace_mode(self.population[i])
